...
function convertTableToHtml(table) {
    ...
    for (let i = 0; i < table.label.length; i++) {
        if (table.label.length === 1) {
            ...
            dataTable += `<div class="col"><table class="table table-borderless ${table.class}"><tr class="deletable"><th class="text-right" contenteditable="true">${table.label[i]}</th><td contenteditable="true">${table.content[i]}</td></tr></table></div>`;
        } else if (i === 0) {
            //First row of the table => open div tag
            dataTable += `<div class="col-6"><table class="table table-borderless table-sm ${table.class}"><tr class="deletable"><th class="text-right" contenteditable="true">${table.label[i]}</th><td contenteditable="true">${table.content[i]}</td></tr>`;
        } else if (i === table.label.length - 1) {
            //Last row of the table => close div tag
            dataTable += `<tr class="deletable"><th class="text-right" contenteditable="true">${table.label[i]}</th><td contenteditable="true">${table.content[i]}</td></tr></table></div>`;
        } else {
            //Every other row
            dataTable += `<tr class="deletable"><th class="text-right" contenteditable="true">${table.label[i]}</th><td contenteditable="true">${table.content[i]}</td></tr>`;
        }
    }
    return dataTable;
}
