...
function convertTableToHtml(table) {
    ...
    for (let i = 0; i < table.label.length; i++) {
        if (table.label.length === 1) {
            ...
            dataTable += `<div class="col ${table.class}"><div class="row deletable"><div class="col-4 text-right info-label" contenteditable="true">${table.label[i]}</div><div class="col-8" contenteditable="true">${table.content[i]}</div></div></div></div>`;
        } else if (i === 0) {
            //First row of the table => open div tag
            dataTable += `<div class="col-6 ${table.class}"><div class="row deletable"><div class="col-4 text-right info-label" contenteditable="true">${table.label[i]}</div><div class="col-8" contenteditable="true">${table.content[i]}</div></div>`;
